...
- **Kinds**: `Change` (modified, carries compensation), `Skip` (already correct), `NotApplicable` (nothing on this machine to act on), `Refused` (Windows refused; not a failure), `Irreversible` (no way back). Only `Change` counts for `DidApplyAnything`, so a run that recorded no change is reported as nothing to do rather than as a failure.
- **The record of an apply**: `OptimizationService` writes what a run did to `%LocalAppData%\optimizerDuck\History\{id}.json` (one file per item, atomic, best effort, never part of the revert data). `ChangeRecordStore` reads it back for the details view, and the applied badge's summary comes from it.
