...
- `optimizerDuck/` — WPF app (single project, no sub-projects):
  - `Domain/` — models, interfaces, attributes (no UI deps)
    ...
    - `Execution/` — `OpCall` (explicit per-operation context: changes, logger, cancellation), `ChangeSet` (thread-safe change collector) + `Change` (one recorded step, also the step result shown to the user), `OpResult` (single-operation result), `OperationSubject` (the Id, key and English log name a run and its record belong to), `OperationRequest` (one run: subject, display name, logger, whether its compensation is persisted) + `RevertPersistence`, `ToolSubjects` (the fixed subjects of the built-in tools)
  ...
  - `Services/` — business logic:
    ...
    - `Optimization/` — `OptimizationRegistry`, `OptimizationService`, `OperationRunner` (the one lifecycle a system-changing run takes: invoke the work, classify the outcome from the recorded steps, write the record, persist revert data when the subject asks for it), `OptimizationValidation` (fail-fast Id checks at startup)
...
- **Single execution path**: `BaseOptimization` + `OptimizationContext` (`OpCall`) + static providers + `ShellService`, orchestrated by `OperationRunner`. Every system-changing run goes through it: an optimization (`OptimizationService.ApplyAsync` builds the `OperationRequest`) and a tool action (`ToolRunPresenter` builds it) differ only in their subject and in whether revert data is persisted. There is no second framework — do not add one, and do not write a second implementation of an existing Windows operation.
...
- **Skip policy**: a provider whose mutation needs a privilege it may not have, has a side effect beyond the value it sets, or can be refused when called a second time reads the current state first and records a skip when the machine already matches, without calling the mutation (`RegistryService`, `ServiceProcessService`, `ScheduledTaskService`, `PowerPlanChanges`, hibernation, USB). An item never decides this on its own: skipping at the item level drops the step from the record entirely.
- **Kinds**: `Change` (modified, carries compensation), `Skip` (already correct), `NotApplicable` (nothing on this machine to act on), `Refused` (Windows refused; not a failure), `Irreversible` (no way back). `DidApplyAnything` (only `Change`, and what revert persistence is gated on) is narrower than `ModifiedSystem` (`Change` + `Irreversible`, and what tells a change from nothing to do), so a run whose only step was irreversible is a success rather than nothing to do, and a run that recorded no change at all is reported as nothing to do rather than as a failure.
- **The record of a run**: `OperationRunner` writes what a run did to `%LocalAppData%\optimizerDuck\History\{id}.json` (one file per subject, atomic, best effort, never part of the revert data). An optimization and a built-in tool both write one, under the fixed ids in `ToolSubjects`. `ChangeRecordStore` reads it back for the details view and for a retry's attempt counts, and the applied badge's summary comes from it.
- **Per-step failure policy**: providers record a failed `Change` with an error and (where a retry can help) a retry action; they do not throw. Power failures record the same way via `PowerPlanChanges` (the service itself only returns diagnostics). Only truly unrecoverable state throws. `OperationRunner` persists partial work with `CancellationToken.None` and builds the `OptimizationResult`; an exception from a run is recorded as a failed step named for the user.
...
- **Retry**: `OptimizationService.RetryFailedStepsWithResultsAsync()` re-invokes `Retry` with a fresh `OpCall` and persists recovered steps via `AppendRevertStepAsync`. The optimization page's dialog and `ToolRunPresenter` both call it; reverting an optimization passes a `RevertManager` and a tool passes none, so a retried tool step writes no revert data.
...
- `ProcessRunner` + `ShellService` come from DI; there is no init step. `OperationRunner` creates the per-run `ChangeSet`. Revert steps that run commands receive the app shell service from `RevertManager` so the configured timeout applies to undo.
- The full build/format/analysis gate is: `dotnet build ... Release` (the build **fails on any warning**: `TreatWarningsAsErrors` + `EnforceCodeStyleInBuild` are set in both project files, so the severity table in `.editorconfig` decides), then `dotnet test`, then `csharpier check .`. `AnalysisMode=All` is deliberately not enabled; the measured reason is in `.editorconfig`.
