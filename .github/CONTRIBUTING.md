...
When writing new optimizations or features, DO NOT perform direct modifications without using our provided services. These services are wrapper classes located in `Services/OptimizationServices/` that automatically handle logging, privilege management, and crucial **Revert** tracking.
...
Optimizations reside in `Core/Optimizers/`. They perform direct system tweaks and return an `ApplyResult`.
...
2. **Select the Target Category**:
   - Check existing categories in `Core/Optimizers/` (e.g., `Performance.cs`, `SecurityAndPrivacy.cs`).
...
4. **Creating a New Category**:
   ...
   - Update the `OptimizationCategoryOrder` in `Core/Models/Optimization/OptimizationCategory.cs` to include your new category enum.

### 🌟 Creating a New Feature

Features reside in `Core/Features/`. Features are typically UI toggles that flip Windows settings ON or OFF dynamically.

1. **Select the Target Feature Category**:
   - Check existing Feature Categories in `Core/Features/` (e.g., `Desktop.cs`, `SystemFeatures.cs`).
...
3. **Creating a New Feature Category**:
   ...
   - Implement `IFeatureCategory` and register its enum in `FeatureCategoryOrder`.
...
2. **Implementation Strategy**:
   - **View**: Create a `.xaml` page in `UI/Views/Pages/`.
   - **ViewModel**: Create a `ViewModel` in `UI/ViewModels/Pages/`. Inherit from `ViewModel` via `CommunityToolkit.Mvvm`.
...
1. Do not touch `Translations.Designer.cs`. Instead, if you use Visual Studio, use this extension [ResXManager](https://marketplace.visualstudio.com/items?itemName=TomEnglert.ResXManager) to open `Resources/Languages/Translations.resx`.
...
3. **Push & Open PR**:
   - Draft a PR description. Outline *what* changed and *why*.
